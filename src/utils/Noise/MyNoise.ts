...
class MyNoise {
  ...
  // static arr = [50, 55, 52, 56, 53, 52, 47, 47, 44, 48, 44, 40, 39, 35, 40, 43, 46, 50, 50, 55, 52, 56, 53, 52, 47, 47, 44, 48, 44, 40, 39, 35, 40, 43, 46, 50, 50, 55, 52, 56, 53, 52, 47, 47, 44, 48, 44, 40, 39, 35, 40, 43, 46, 50, 50, 55, 52, 56, 53, 52, 47, 47, 44, 48, 44, 40, 39, 35, 40, 43, 46, 50, 50, 55, 52, 56, 53, 52, 47, 47, 44, 48, 44, 40, 39, 35, 40, 43, 46, 50, 50, 55, 52, 56, 53, 52, 47, 47, 44, 48, 44, 40, 39, 35, 40, 43, 46, 50, 50, 55, 52, 56, 53, 52, 47, 47, 44, 48, 44, 40, 39, 35, 40, 43, 46, 50]
  static arr: number[] = []
  // .slice(150, 180)
  static dispersion = .1
  ...
  static cubicHermite(a: number, b: number, c: number, d: number, t: number) :number {
      let aPolynomial = -a/2.0 + (3.0*b)/2.0 - (3.0*c)/2.0 + d/2.0;
      let bPolynomial = a - (5.0*b)/2.0 + 2.0*c - d / 2.0;
      let cPolynomial = -a/2.0 + c/2.0;
  ...
  static noise(n: number, smoothness?: number) :number {
    ...
    let t = indexFound - nearestIndex

    let b = this.arr[nearestIndex]
    let c = this.arr[nearestIndex + 1]
    let a = this.arr[nearestIndex - 1] || b - (c - b)
    let d = this.arr[nearestIndex + 2] || c - (b - c)

    let ret = this.cubicHermite(a, b, c, d, t)   
    return ret
  }
